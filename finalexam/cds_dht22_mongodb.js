...
// MongoDB connection
mongoose.connect('mongodb://localhost:27017/iot10'); // DB name
...
sp.on('data', function (data) { // call back when data is received
    ...
    seconedcommadix =readData.indexOf(',',firstcommaidx+1) + 1 ;
 ...
 if (readData.lastIndexOf(',') > firstcommaidx && firstcommaidx > 0) {
        ...
        humi = readData.substring(readData.indexOf(',',firstcommaidx+1) + 1, readData.indexOf(',',seconedcommadix));
        lux = readData.substring(readData.indexOf(',',seconedcommadix)+1, readData.lastIndexOf(','));
        jin = readData.substring(readData.lastIndexOf(',')+1);
        ...
        mdata[4]=jin;
        //console.log(mdata);
        var iot = new Sensor({date:dStr, temperature:temp, humidity:humi, luminosity:lux, jin:jin});
